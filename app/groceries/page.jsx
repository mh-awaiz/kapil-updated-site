...
const SUBCATEGORIES = [
  ...
  { id: "snacks_drinks", label: "Snacks & Drinks", emoji: "🍿" },
  { id: "beauty_personal_care", label: "Beauty & Care", emoji: "💄" },
  { id: "home_lifestyle", label: "Home & Lifestyle", emoji: "🏠" },
  { id: "food_veg", label: "Food – Veg", emoji: "🥦" },
  { id: "food_nonveg", label: "Food – Non Veg", emoji: "🍗" },
];
...
export default function GroceriesPage() {
  ...
  const filtered = useMemo(() => {
    let list = activeTab === "all" ? products : products.filter((p) => p.subcategory === activeTab);
    if (query) {
      const fuse = new Fuse(list, { keys: ["title", "description"], threshold: 0.3 });
    ...
    <div className="min-h-screen bg-[#22323c]">
      ...
      <div className="bg-[#1a2830] pt-28 pb-12 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="inline-block px-3 py-1 rounded-full bg-[#17d492]/10 border border-[#17d492]/20 text-[#17d492] text-xs font-black tracking-widest uppercase mb-4">
            🛒 New Section
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
            Groceries & <span className="text-[#17d492]">Essentials</span>
          </h1>
          <p className="text-slate-400 mt-3 font-medium">Fast delivery on everyday essentials</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-8">
          {SUBCATEGORIES.map((cat) => (
            <button
              key={cat.id}
              onClick={() => { setActiveTab(cat.id); setVisibleCount(PRODUCTS_PER_PAGE); }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.slice(0, visibleCount).length > 0 ? (
                ...
                <div className="col-span-full flex flex-col items-center justify-center py-20 bg-[#1a2830] rounded-3xl border border-dashed border-slate-700">
                  ...
                  <p className="text-xl font-black text-slate-400">No products found</p>
                  <p className="text-slate-600 text-sm mt-1">Check back soon — we're stocking up!</p>
                  {query && (
                    <button onClick={() => setQuery("")} className="mt-4 text-[#17d492] text-sm font-black underline">
                      Clear Search
                    </button>
                  )}
                </div>
